...
export class MeasuredText {
  ...
  private measureWrappedText(): WrappedLine[] {
    const wrappedText = containsMultiCodePointGrapheme(this.text)
      ? wrapByGrapheme(this.text, this.columns)
      : wrapAnsi(this.text, this.columns, {
          hard: true,
          trim: false,
        })
...
function containsMultiCodePointGrapheme(text: string): boolean {
  for (const { segment } of getGraphemeSegmenter().segment(text)) {
    if (segment.length !== firstGrapheme(segment).length || Array.from(segment).length > 1) return true
  }
  return false
}

function wrapByGrapheme(text: string, columns: number): string {
  const output: string[] = []
  for (const logicalLine of text.split('\n')) {
    ...
    for (const { segment } of getGraphemeSegmenter().segment(logicalLine)) {
      const segmentWidth = stringWidth(segment)
      if (current && width + segmentWidth > columns) {
        output.push(current)
        current = ''
        width = 0
      }
