...
export function normalizedGraphemeOffset(text: string, offset: number): number {
  ...
  if (clamped === text.length) boundary = text.length
  return text.slice(0, boundary).normalize('NFC').length
}
